...
- every axiom must map to one of the six proof obligations, every obligation must be
...
1. Copy `formal/templates/attempt/` to a new immutable directory such as
   `formal/attempts/LATT-0001-first-principles/`. Never overwrite a competing attempt.
2. Fill `manifest.json`, `Claim.lean`, and `Model.lean`.
3. Run `python3 -m pure_tate lean-check --attempt LATT-0001 --write`.
...
5. Run `python3 -m pure_tate lean-status` and `python3 -m pure_tate lean-audit`.
