...
-- LEAN-MODEL-WITNESS The LC66-002 hash-pinned C66Vocabulary signature is embedded verbatim. modelVocabulary is a concrete instance with genuinely different ClosedPureIndex and OpenPureIndex types. Restriction records only closed/open alignment; Kunneth splitting separately records diagonal weight and positive factor count; containment records positive marking count; polarizability records diagonal weight; and quotient Tate type records only the Tate index equation. All 34 mathematical axiom schemas are proved for this single concrete vocabulary.
-- LEAN-NONCOLLAPSE AX02 explicitly instantiates its universal Kunneth premise and combines it with aligned restriction and positive-factor splitting. AX31 separately combines containment, polarizability, and quotient Tate type; the quotient predicate is not definitionally equal to the full Tate predicate. Wrong factor count, zero markings, off-diagonal weight, quotient-without-polarizability, the CKgP boundary, vcd threshold, BM Tate index, and duality twist all fail in explicit witnesses.
...
def modelVocabulary : C66Vocabulary where
  RestrictionSurjectsOnLowestWeight := aligned
  KunnethSplittingByChowKunnethProjectors := fun src => src.degree = src.weight ∧ 1 <= src.factors
  ...
  KunnethSummandRestrictsIntoPsi := fun idx tgt =>
    aligned idx.source tgt ∧
    (1 <= idx.source.factors -> 1 <= tgt.space.markings) ∧
    (summandVanishes idx ∨ hasTwo idx.slots = true)
  ...
  ContainedInPsi := fun p => 1 <= p.space.markings
  ...
  PureIsFiniteTateSum := fun p t =>
    (p.weight + 2 * t = 0 ∧ p.degree = p.weight ∧
      p.space.genus = 6 ∧ p.space.markings <= 5) ∨
    (p.weight + 2 * t = 0 ∧ p.degree = p.weight ∧
      1 <= p.space.markings)
  PhiIsFiniteTateSum := fun p t => p.weight + 2 * t = 0 ∧ p.degree = p.weight
  PsiIsQuotientOfFiniteTateSum := fun p t => p.weight + 2 * t = 0
...
theorem lenOf_replicate (n : Nat) (slot : Slot) :
    lenOf (List.replicate n slot) = n := by
  induction n with
  | zero => rfl
  | succ n ih => simp [List.replicate, lenOf, ih, Nat.add_comm]

theorem ax01 : AX01 := by intro src tgt h1 h2 h3 h4 _; exact ⟨h1,h2,h3,h4⟩
theorem ax02 : AX02 := by
  intro src tgt hrestrict hsummands hsplit
  let probe : KunnethIndex := {
    source := src
    slots := List.replicate (Int.toNat src.factors) Slot.zero
  }
  have hprobe := hsummands probe rfl (by exact lenOf_replicate _ _)
  dsimp [V, modelVocabulary] at hrestrict hsplit hprobe ⊢
  exact hprobe.2.1 hsplit.2
...
theorem ax06 : AX06 := by intro _ _ _ _ _ _ _ hpos hdiag; exact ⟨hdiag,hpos⟩
...
theorem ax18 : AX18 := by
  intro idx tgt h1 h2 h3 h4 hv
  exact ⟨⟨h1,h2,h3,h4⟩, (by omega), Or.inl hv⟩
theorem ax19 : AX19 := by
  intro idx _ _ pre post tgt _ _ _ hs _ _ _ _ _ h1 h2 h3 h4 _ _ _ _ _ _ _
  exact ⟨⟨h1,h2,h3,h4⟩, (by omega), Or.inr (by rw [hs]; exact hasTwo_split pre post)⟩
...
theorem ax26 : AX26 := by
  intro p c hcycle _ hd hw
  left
  dsimp [V, modelVocabulary] at hcycle ⊢
  exact ⟨by omega, hw.symm, hcycle.2.1, hcycle.2.2⟩
...
theorem ax31 : AX31 := by
  intro _ _ hcontained hpolarizable hquotient
  right
  exact ⟨hquotient, hpolarizable, hcontained⟩
...
def quotientOnly : OpenPureIndex := { space := { genus := 6, markings := 6 }, degree := 25, weight := 26 }
def zeroMarked : OpenPureIndex := { space := { genus := 6, markings := 0 }, degree := 26, weight := 26 }
def sourceZero : ClosedPureIndex := { genus := 6, factors := 0, degree := 26, weight := 26 }
...
def NC03 : Prop := ¬ V.PurePolarizableOfWeight offDiagonal
...
def NC10 : Prop := ¬ V.ContainedInPsi zeroMarked
def NC11 : Prop := V.PsiIsQuotientOfFiniteTateSum quotientOnly (-13) ∧ ¬ V.PureIsFiniteTateSum quotientOnly (-13)
def NC12 : Prop := ¬ V.KunnethSplittingByChowKunnethProjectors sourceZero
...
theorem nc10 : NC10 := by dsimp [NC10, V, modelVocabulary, zeroMarked]; decide
theorem nc11 : NC11 := by dsimp [NC11, V, modelVocabulary, quotientOnly]; decide
theorem nc12 : NC12 := by dsimp [NC12, V, modelVocabulary, sourceZero]; decide
...
theorem c66_model_is_consistent_and_materially_noncollapsing :
  ...
  ⟨ax01,ax02,ax03,ax04,ax05,ax06,ax07,ax08,ax09,ax10,ax11,ax12,ax13,ax14,ax15,ax16,
   ...
   ax33,ax34,nc01,nc02,nc03,nc04,nc05,nc06,nc07,nc08,nc09,nc10,nc11,nc12⟩
