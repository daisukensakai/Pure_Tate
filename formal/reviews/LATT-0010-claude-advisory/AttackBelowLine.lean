...
-- ATTACK 3: delete the below-critical-line route (G7D16-OBL-02 / AX15).
...
theorem ax14 : AX14 := fun _ e h1 h2 h3 h4 h5 => ⟨rfl, h1, Or.inr ⟨h2, e, h3, h4, h5⟩⟩
...
theorem attack3_models_all_but_ax15 :
...
theorem attack3_refutes_ax15 : ¬ AX15 := by
  intro h
  have hv := h stable60 7 stable60Stable (by decide) (by decide) (by decide) (by decide)
    (by decide) ⟨stable60Stable, by decide, by decide, by decide⟩
    ⟨rfl, rfl, rfl, by decide,
      (by simp [openRangeIonelIndex, stable60]), (fun _ => by decide)⟩
  rcases hv.2.2 with hw | hc
  · exact absurd hw.2.2.2 (by decide)
  · exact absurd hc.1 (by decide)

theorem attack3_target_fails_at_60 :
    ¬ CompactH16IsFiniteTateSum (compactH16Target stable60) := by
  refine compactTargetFailsWithoutOpen openTateModel stable60 (by decide) ?_
  rintro (⟨_, _, hw | hc⟩ | ⟨_, hp, _⟩)
  · exact absurd hw.2.2.2 (by decide)
  · exact absurd hc.1 (by decide)
  · exact absurd hp.2.2.2.1 (by decide)

#print axioms attack3_models_all_but_ax15
#print axioms attack3_refutes_ax15
#print axioms attack3_target_fails_at_60
