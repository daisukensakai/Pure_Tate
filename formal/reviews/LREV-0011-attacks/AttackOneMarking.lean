...
-- ATTACK: drop the one-marking primitive-quotient disjunct from CriticalEndpointVanishes.
-- Everything else in the model is verbatim from LATT-0009 Model.lean.
...
-- ATTACK: make the target predicate track the assembly instead of asserting the range.
-- Genus <= 2 is a base input; every other pair must come through the boundary extension.
...
theorem ax10 : AX10 := fun _ h1 h2 h3 h4 => ⟨h1, h2, h3, h4⟩
...
-- (1) The attacked model satisfies every campaign axiom EXCEPT the one-marking route.
theorem attack_models_all_but_ax09 :
    AX01 ∧ AX02 ∧ AX03 ∧ AX04 ∧ AX05 ∧ AX06 ∧ AX07 ∧ AX08 ∧ AX10 ∧
...
-- (2) AX09 itself genuinely fails here: it is the premise we removed.
theorem attack_refutes_ax09 : ¬ AX09 := by
  intro h
  have hcoh : EndpointArithmeticIsCoherent endpointG6 := by decide
  have hpts : pureTautSmallerPointedModel endpointG6 := ⟨hcoh, by decide⟩
  have hionel : ionelModel (psiSourceIonelIndex endpointG6) :=
    ⟨rfl, rfl, rfl, by decide, fun _ => by decide, fun _ => by decide⟩
  have := h endpointG6 (by right; right; right; left; rfl) rfl
    ⟨hcoh, by decide⟩ ⟨hcoh, by decide⟩ ⟨hcoh, by decide, rfl, hpts⟩
    ⟨hcoh, by decide, hpts, hionel⟩
  exact absurd this.2.1 (by decide)

-- (3) Without AX09 the target FAILS at (6,6): the endpoint derivation is load-bearing.
theorem attack_target_fails_at_66 :
    ¬ CompactH16IsFiniteTateSum (compactH16Target stable66) := by
  rintro ⟨_, _, (hg | hh)⟩
  · exact absurd hg (by decide)
  · obtain ⟨s, b, o, _, hsh, hso, hr, _, _, ho, _⟩ := hh.1
    have hsp : s.pair = stable66 := by
      ...
      rw [hsh] at hn2
      exact hn2.symm
    have hopair : o.pair = stable66 := by
    ...
    · rcases hz.2.2 with hw | ⟨_, e, hgg, _, hcv⟩ | hb
      · rw [hopair] at hw; exact absurd hw.2.2.2 (by decide)
      · rw [hopair] at hgg
        rcases hcv.1 with rfl | rfl | rfl | rfl | rfl
        · exact absurd hgg (by decide)
        · exact absurd hgg (by decide)
        · exact absurd hgg (by decide)
        · exact absurd hcv.2.1 (by decide)
        · exact absurd hgg (by decide)
      · rw [hopair] at hb; exact absurd hb.2.2.2.1 (by decide)
    · rw [hopair] at hlow; exact absurd hlow.2.1.2.2.2.1 (by decide)

#print axioms attack_models_all_but_ax09
#print axioms attack_refutes_ax09
#print axioms attack_target_fails_at_66
