...
def main():
    ...
    if kind == "finding-audit":
        ...
        msg = f"C66-001 • {aid} {job.get('finding_id')} verdict={payload.get('verdict')}"
    elif kind == "review":
        ...
        # validate cross-engine before publish
        if payload.get("engine") == task.get("paired_source_engine"):
            raise SystemExit("cross-engine check failed post-hoc")
        ...
        msg = f"C66-001 • {aid} {job.get('subproblem_id')} status={payload.get('status')}"
    job["status"] = "done"; _save(vault, job)
    notify(f"Pure Tate • {engine} {job['kind']} done", msg)
    print("done", aid, job.get("result_verdict") or job.get("result_status") or "published", flush=True)
