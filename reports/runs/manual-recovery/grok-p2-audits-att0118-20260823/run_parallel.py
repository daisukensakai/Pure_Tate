#!/usr/bin/env python3
"""Parallel xAI Grok: ATT-0118 P2 plus finding-audits of FND-0169..0172.

One campaign lock; five concurrent run_task calls. Not C66-FULL. No model fallback.
"""
...
from pure_tate.tasking import finding_audit_tasks, review_tasks

CAMPAIGN_ID = "C66-001"
ENGINE = "grok"
TIMEOUT = 10800
VAULT = Path(__file__).resolve().parent
P2_TASK_ID = "TASK-V-ATT-0118-P2"
AUDIT_FINDING_IDS = ("FND-0169", "FND-0170", "FND-0171", "FND-0172")
...
def main() -> int:
    ...
    matches = [task for task in review_tasks() if task.get("id") == P2_TASK_ID]
    if len(matches) != 1:
        raise RuntimeError("expected exactly one %s" % P2_TASK_ID)
    p2 = matches[0]
    if p2.get("status") != "ready" or int(p2.get("review_pass") or 0) != 2:
        raise RuntimeError("%s is not ready P2: %s" % (P2_TASK_ID, p2.get("status")))
    if ENGINE in set(p2.get("excluded_reviewer_engines") or []):
        raise RuntimeError("%s is excluded from %s" % (ENGINE, P2_TASK_ID))
    ...
    try:
        with CampaignRunLock(CAMPAIGN_ID):
            ...
            ledger, ledger_path = _new_run_ledger(
                CAMPAIGN_ID, n_jobs, ["grok"], ["codex"], ["grok", "qwen"]
            )
            jobs = []
            events = []
            ledger_lock = threading.Lock()

            def add_job(kind, task, prefix, directory, extra):
                task = dict(task)
            ...
            p2_task, p2_job, p2_event, p2_out, p2_res = add_job(
                "review",
                p2,
                "REV",
                ROOT / "proof" / "reviews",
                {
                    "target_attempt_id": "ATT-0118",
                    "review_pass": 2,
                    "finding_id": None,
                },
            )
            ...
            launch = {
                "note": (
                    "parallel xAI Grok: ATT-0118 P2 plus FAUD of FND-0169..0172; "
                    "official P2 first is cursor-grok; no model fallback"
                ),
                "jobs": jobs,
            ...
            notify(
                "Pure Tate • Grok P2 + 4 audits starting",
                "%s • %s plus %s"
                % (
                    CAMPAIGN_ID,
                    p2_job["artifact_id"],
                    ", ".join(spec[1]["artifact_id"] for spec in audit_specs),
                ),
            )
            ...
            def run_one(task, job, event, output, reservation_path):
                ...
                try:
                    ...
                    if task["phase"] == "review":
                        ...
                        attempt = load_json(
                            ROOT / "proof" / "attempts" / "ATT-0118.json"
                        )
                        attempt["_path"] = str(
                            ROOT / "proof" / "attempts" / "ATT-0118.json"
                        )
                        touched = record_review_findings(artifact, attempt)
                        job["ingested_findings"] = [item.get("id") for item in touched]
            ...
            work = [(p2_task, p2_job, p2_event, p2_out, p2_res)] + audit_specs
            failures = 0
        ...
        notify("Pure Tate • Grok parallel blocked", str(exc))
