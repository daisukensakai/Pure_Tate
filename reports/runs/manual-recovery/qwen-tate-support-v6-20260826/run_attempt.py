#!/usr/bin/env python3
"""Run one normal Qwen attempt for the v6 C66-TATE-SUPPORT task."""
...
def current_task() -> dict:
    ...
    if dependencies.get("C66-SUPPORT-INTERFACE") != "ATT-0120":
        raise RuntimeError("verified support interface ATT-0120 is not attached")
    if dependencies.get("C66-SUPPORT-FILTRATION") != "ATT-0130":
        raise RuntimeError("verified support-filtration ATT-0130 is not attached")
...
def main() -> int:
    with CampaignRunLock(CAMPAIGN_ID):
        ...
        task = current_task()
        ledger, ledger_path = _new_run_ledger(CAMPAIGN_ID, 1, [ENGINE], [ENGINE], ["claude", "codex"])
