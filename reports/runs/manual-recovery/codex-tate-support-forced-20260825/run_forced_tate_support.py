#!/usr/bin/env python3
"""PI override: Codex forced-prompt C66-TATE-SUPPORT with 2 parallel Grok workers.

Same conditions as the earlier Codex forced TATE-SUPPORT turn: completeness
contract retargeted at TASK-C66-M-005, not official C66-FULL forced-proof.
Worker caps: 2 concurrent identities, 256 turns each. Codex controller
max_requests raised to 16. Default engines.json stays 1/4.
"""
...
FORCED_PROMPT = (
    "reports/runs/manual-recovery/codex-tate-support-forced-20260825/FORCED_TATE_SUPPORT.md"
)
...
def main() -> int:
    launch = {
        "note": (
            "PI override: Codex forced-prompt TATE-SUPPORT (not C66-FULL); "
    ...
    try:
        with CampaignRunLock(CAMPAIGN_ID):
            ...
            for item in campaign.get("subproblems") or []:
                if item.get("id") == SUBPROBLEM_ID:
                    ...
                    break
            if not isinstance(assigned, str) or "T_Z" not in assigned:
            ...
            _notify(
                "Pure Tate • Codex forced TATE-SUPPORT starting",
        ...
        launch["error"] = str(exc)
        _save_launch(launch)
        _notify("Pure Tate • forced TATE-SUPPORT blocked", str(exc))
