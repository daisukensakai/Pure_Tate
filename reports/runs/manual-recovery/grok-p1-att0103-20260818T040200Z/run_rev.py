#!/usr/bin/env python3
"""One Grok P1 review of ATT-0103. Official drive would select Qwen first."""
...
from pure_tate.paired import ArtifactValidationError, recover_attempt_from_trace
...
CAMPAIGN_ID = "C66-001"
TASK_ID = "TASK-V-ATT-0103-P1"
TARGET_ATTEMPT = "ATT-0103"
ENGINE = "grok"
TIMEOUT = 10800
    ...
    try:
        with CampaignRunLock(CAMPAIGN_ID):
            ...
            ledger, ledger_path = _new_run_ledger(
                CAMPAIGN_ID, 1, ["grok"], ["claude"], [ENGINE, "qwen"]
            )
            run_id = ledger["run_id"]
            artifact_id, reservation_path = reserve_prefixed_artifact(
                ROOT / "proof" / "reviews", "REV", run_id
            )
            if artifact_id == "REV-0151":
                raise RuntimeError("refusing to reuse spent REV-0151")
            ...
            launch = {
                "note": "Grok P1 of ATT-0103; Qwen REV-0151 closed; no model fallback",
            ...
            notify(
                "Pure Tate • Grok GEO-COMP P1 starting",
        ...
        notify("Pure Tate • Grok P1 blocked", str(exc))
