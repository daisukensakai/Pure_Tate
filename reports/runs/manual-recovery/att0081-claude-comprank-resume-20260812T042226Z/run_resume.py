#!/usr/bin/env python3
"""Resume interrupted Claude COMP-RANK into ATT-0084."""
...
def main() -> int:
    ...
    if (ROOT / "proof/attempts/ATT-0081.json").exists():
        raise SystemExit("refusing: ATT-0081 unexpectedly exists")
    if output.exists():
        raise SystemExit("refusing to overwrite existing %s" % output)
    if artifact_id == "ATT-0081":
        raise SystemExit("refusing to write into spent ATT-0081")
    ...
    notify(
        ...
        "C66-001 • Claude --resume %s\nTASK-C66-M-008 -> %s (ATT-0081 closed)"
    ...
    if proc.returncode != 0 or not raw.strip():
        spend_artifact_reservation(reservation, reason="resume_agent_failure", task_id="TASK-C66-M-008")
    ...
    try:
        ...
        spend_artifact_reservation(reservation, reason="resume_extract_failure", task_id="TASK-C66-M-008")
        notify("Pure Tate • Claude COMP-RANK resume extract failed", str(exc))
        raise
    artifact["id"] = artifact_id
    artifact["engine"] = "claude"
    artifact["task_id"] = "TASK-C66-M-008"
    artifact["subproblem_id"] = "C66-COMP-RANK"
