#!/usr/bin/env python3
"""Serial finding-audits: Codex on FND-0154, then Qwen on FND-0155.

FND-0153 stays queued for a later Claude resume (FAUD-0163 spent).
"""
...
CAMPAIGN_ID = "C66-001"
JOBS = (
    {"finding_id": "FND-0154", "engine": "codex"},
    {"finding_id": "FND-0155", "engine": "qwen"},
)
TIMEOUT = 10800
    ...
    try:
        with CampaignRunLock(CAMPAIGN_ID):
            ...
            ledger, ledger_path = _new_run_ledger(
                CAMPAIGN_ID, 2, ["codex", "qwen"], ["grok"], ["codex", "qwen"]
            )
            launch = {
                "note": "serial audits: Codex FND-0154 then Qwen FND-0155; FND-0153 reserved for Claude",
                "jobs": [],
            ...
            for index, task in enumerate(planned, start=1):
                ...
                if artifact_id == "FAUD-0163":
                    raise RuntimeError("refusing to reuse spent FAUD-0163")
                ...
                notify(
                    "Pure Tate • %s finding-audit starting" % engine,
                ...
                try:
                    ...
                    event["notification_delivery"] = notify_campaign_step(
                        CAMPAIGN_ID, event, 2, desktop=True, ntfy=True
                    )
                    ledger["events"] = events
                    _write_run_ledger(ledger_path, ledger)
                    atomic_write_json(VAULT / "LAUNCH.json", launch)
        ...
        notify("Pure Tate • audits blocked", str(exc))
