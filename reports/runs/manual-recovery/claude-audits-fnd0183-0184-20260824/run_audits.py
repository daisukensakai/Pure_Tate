#!/usr/bin/env python3
"""Serial Claude finding-audits of FND-0183 then FND-0184.

FND-0186 excludes Claude. No model fallback.
"""
...
CAMPAIGN_ID = "C66-001"
JOBS = (
    {"finding_id": "FND-0183", "engine": "claude"},
    {"finding_id": "FND-0184", "engine": "claude"},
)
TIMEOUT = 10800
    ...
    try:
        with CampaignRunLock(CAMPAIGN_ID):
            ...
            ledger, ledger_path = _new_run_ledger(
                CAMPAIGN_ID, STEPS, ["grok"], ["claude"], ["claude", "grok"]
            )
            launch = {
                "note": "serial Claude FAUD of FND-0183 then FND-0184; FND-0186 excludes Claude; no model fallback",
                "jobs": [],
            ...
            for index, task in enumerate(planned, start=1):
                ...
                notify(
                    "Pure Tate • Claude finding-audit starting",
        ...
        notify("Pure Tate • Claude audits blocked", str(exc))
