# C66-GEO-COMP forced complete resolution
...
subproblem `C66-GEO-COMP`, not the campaign Hodge/Tate group RED-0001.

Prove or disprove the exact `C66-GEO-COMP` theorem stated in `TASK.json` as
`exact_theorem`. Return a complete, gap-free argument that resolves
`C66-GEO-COMP`: components, dimensions, stabilizers, and finite-cover behavior
of the evaluation-failure locus `Z`.
...
GEO-COMP frontier obligations as `gap_markers`. Keep working—using both
parallel Grok workers as needed—until the GEO-COMP lemma is complete.

This lemma does **not** resolve the campaign exact target
`W_{-16}H^{BM}_{16}(M_{6,6};Q)`. Set `resolves_exact_target` to false. Do not
claim Hodge, weight, purity, or Tate type for RED-0001.
...
- You have two Grok worker identities. Dispatch both and let them run in
  parallel on distinct narrow GEO-COMP sub-tasks (for example: type-(ii)
  monodromy image versus balanced-base / component-count).
