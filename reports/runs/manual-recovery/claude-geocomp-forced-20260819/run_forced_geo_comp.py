#!/usr/bin/env python3
"""PI override: Claude forced-prompt C66-GEO-COMP with 2 parallel Grok workers.

Uses the forced-turn completeness contract, retargeted at GEO-COMP
(TASK-C66-M-003). Does *not* dispatch official C66-FULL forced-proof: that
artifact must match the campaign Hodge/Tate theorem and would be rejected as
a GEO-COMP lemma.

Worker caps: 2 concurrent identities, 256 turns each (harness has no infinite
turn mode). Default engines.json stays 1/4.
"""
...
def main() -> int:
    launch = {
        "note": (
            "PI override: Claude forced-prompt GEO-COMP (not C66-FULL); "
            "2 parallel Grok workers; 256 turns/identity; completeness contract"
        ),
        "stamp": "20260819T070000Z",
    ...
    try:
        with CampaignRunLock(CAMPAIGN_ID):
            ...
            _notify(
                "Pure Tate • Claude forced GEO-COMP starting",
