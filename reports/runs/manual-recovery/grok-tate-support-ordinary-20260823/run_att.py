#!/usr/bin/env python3
"""One ordinary xAI Grok C66-TATE-SUPPORT mathematics turn.

PI override of rotation (official first is cursor-grok). Targets the rewritten
T_16(Z) exact theorem after ATT-0118 quarantine. Not C66-FULL, not forced.
No model fallback.
"""
...
def main() -> int:
    ...
    if not isinstance(exact_theorem, str) or "T_16(Z)" not in exact_theorem:
        raise RuntimeError("%s missing rewritten T_16(Z) exact_theorem" % TASK_ID)
    ...
    try:
        with CampaignRunLock(CAMPAIGN_ID):
            ...
            ledger, ledger_path = _new_run_ledger(
                CAMPAIGN_ID, 1, ["grok"], [ENGINE], ["claude", "cursor-grok"]
            )
            ...
            launch = {
                "note": (
                    "ordinary xAI grok TATE-SUPPORT; rewritten T_16(Z) theorem; "
                    "ATT-0118 quarantined; not C66-FULL; not forced-proof; no model fallback"
                ),
            ...
            notify(
                "Pure Tate • Grok TATE-SUPPORT starting",
        ...
        notify("Pure Tate • Grok TATE-SUPPORT blocked", str(exc))
