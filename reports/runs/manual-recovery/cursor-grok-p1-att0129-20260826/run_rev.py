#!/usr/bin/env python3
"""Run Cursor-Grok's independent P1 review of ATT-0129."""
...
def current_task() -> dict:
    ...
    matches = [task for task in review_tasks(ATTEMPT_ID) if task.get("id") == TASK_ID]
    if len(matches) != 1 or matches[0].get("status") != "ready":
        raise RuntimeError("ATT-0129 has no ready P1 review task")
    task = dict(matches[0])
    if task.get("target_attempt_id") != ATTEMPT_ID:
        raise RuntimeError("P1 task points to the wrong attempt")
    if ENGINE in set(task.get("excluded_reviewer_engines") or []):
        raise RuntimeError("Cursor-Grok is not an independent reviewer")
...
def main() -> int:
    with CampaignRunLock(CAMPAIGN_ID):
        ...
        task = current_task()
        ledger, ledger_path = _new_run_ledger(CAMPAIGN_ID, 1, [ENGINE], ["codex"], ["claude", "grok"])
        ...
        launch = {
            ...
            "note": "Cursor-Grok P1 review of Codex ATT-0129; no xAI-Grok fallback.",
            "status": "running",
        }
        atomic_write_json(launch_path, launch)
        event = {
            "step": 1, "phase": "review", "task_id": TASK_ID,
            "engine": ENGINE, "output": str(output.relative_to(ROOT)),
            "target_attempt_id": ATTEMPT_ID, "review_pass": 1,
        ...
        try:
            ...
            event.update({
                ...
                "review_id": review.get("id"), "verdict": review.get("verdict"),
