...
META = Path("/tmp/pure-tate-three-math-meta.json")
...
def main():
    ...
    run_manifest = vault / f"run-{aid}.json"
    log = vault / f"{aid}.{engine.replace('cursor-','')}.console.log"
    job["status"] = "running"
    _save(vault, job)
    notify(f"Pure Tate • {engine} math starting", f"C66-001 • {task_id} {job['subproblem_id']} -> {aid}")
    with log.open("w") as fh:
        proc = subprocess.run(
            [sys.executable, "-m", "pure_tate", "agent-run",
             "--manifest", str(run_manifest), "--task-id", task_id,
             "--engine", engine, "--output", output, "--timeout", "10800"],
    ...
    if proc.returncode != 0 or not outp.exists():
        ...
        job["status"] = "failed"; job["exit_code"] = proc.returncode
        _save(vault, job)
        notify(f"Pure Tate • {engine} math failed", f"C66-001 • {aid} exit={proc.returncode}")
        raise SystemExit(proc.returncode or 1)
    payload = json.loads(outp.read_text())
    release_artifact_reservation(res)
    job["status"] = "done"
    job["result_status"] = payload.get("status")
    _save(vault, job)
    notify(f"Pure Tate • {engine} math done", f"C66-001 • {aid} {job['subproblem_id']} status={payload.get('status')}")
    print("done", aid, payload.get("status"), flush=True)
