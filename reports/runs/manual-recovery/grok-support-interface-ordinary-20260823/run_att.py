#!/usr/bin/env python3
"""One ordinary xAI Grok C66-SUPPORT-INTERFACE mathematics turn.

PI override of rotation (official first is cursor-grok). Not C66-FULL, not
forced, no TATE-SUPPORT, no model fallback.
"""
...
SUBPROBLEM_ID = "C66-SUPPORT-INTERFACE"
ENGINE = "grok"
TIMEOUT = 10800
...
def main() -> int:
    ...
    if not isinstance(exact_theorem, str) or "K_Z" not in exact_theorem:
        raise RuntimeError("%s missing SUPPORT-INTERFACE exact_theorem" % TASK_ID)
    ...
    try:
        with CampaignRunLock(CAMPAIGN_ID):
            ...
            launch = {
                "note": (
                    "ordinary xAI grok SUPPORT-INTERFACE; TATE-SUPPORT blocked; "
                    "not C66-FULL; not forced-proof; no model fallback"
                ),
            ...
            notify(
                "Pure Tate • Grok SUPPORT-INTERFACE starting",
        ...
        notify("Pure Tate • Grok SUPPORT-INTERFACE blocked", str(exc))
