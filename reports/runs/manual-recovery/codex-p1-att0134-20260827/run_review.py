#!/usr/bin/env python3
"""Run Codex P1 review of Claude ATT-0134."""
...
def main() -> int:
    with CampaignRunLock(CAMPAIGN_ID):
        ...
        if len(matches) != 1 or matches[0].get("status") != "ready":
            raise RuntimeError("ATT-0134 has no ready P1 review")
        task = dict(matches[0])
        if ENGINE in set(task.get("excluded_reviewer_engines") or []):
            raise RuntimeError("Codex is not independent")
        task["selected_engine"] = ENGINE
        ledger, ledger_path = _new_run_ledger(CAMPAIGN_ID, 1, ["cursor-grok"], [ENGINE], [ENGINE])
