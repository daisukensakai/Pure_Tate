#!/usr/bin/env python3
"""Run one normal Claude attempt for the repaired v6 C66-TATE-SUPPORT task."""
...
def current_task() -> dict:
    ...
    if (
        dependencies.get("C66-SUPPORT-INTERFACE") != "ATT-0120"
        or dependencies.get("C66-SUPPORT-FILTRATION") != "ATT-0130"
    ):
...
def main() -> int:
    with CampaignRunLock(CAMPAIGN_ID):
        ...
        ledger, ledger_path = _new_run_ledger(
            CAMPAIGN_ID, 1, ["cursor-grok"], [ENGINE], ["cursor-grok"]
        )
