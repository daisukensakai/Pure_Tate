#!/usr/bin/env python3
"""One Claude P1 review of ATT-0123.

Official first reviewer after excluding prover codex is cursor-grok; PI
override to claude. No model fallback.
"""
...
CAMPAIGN_ID = "C66-001"
TASK_ID = "TASK-V-ATT-0123-P1"
TARGET_ATTEMPT = "ATT-0123"
ENGINE = "claude"
TIMEOUT = 10800
...
def main() -> int:
    ...
    if task.get("review_pass") != 1:
        raise RuntimeError("expected review_pass 1, got %s" % task.get("review_pass"))
    ...
    try:
        with CampaignRunLock(CAMPAIGN_ID):
            ...
            ledger, ledger_path = _new_run_ledger(
                CAMPAIGN_ID, 1, ["grok"], ["grok"], [ENGINE, "codex"]
            )
            ...
            launch = {
                "note": (
                    "Claude P1 of ATT-0123 Codex TATE-SUPPORT proposed lemma; "
                    "official first is cursor-grok; PI override; no model fallback"
                ),
            ...
            notify(
                "Pure Tate • Claude ATT-0123 P1 starting",
        ...
        notify("Pure Tate • Claude ATT-0123 P1 blocked", str(exc))
