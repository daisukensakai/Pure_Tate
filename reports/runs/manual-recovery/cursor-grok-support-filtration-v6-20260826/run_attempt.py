#!/usr/bin/env python3
"""Run one normal Cursor-Grok attempt for C66-SUPPORT-FILTRATION (v6)."""
...
CAMPAIGN_ID = "C66-001"
SUBPROBLEM_ID = "C66-SUPPORT-FILTRATION"
ENGINE = "cursor-grok"
TIMEOUT = 10800
...
def current_task() -> dict:
    ...
    if len(matches) != 1 or matches[0].get("status") != "ready":
        raise RuntimeError("C66-SUPPORT-FILTRATION is not uniquely ready")
    task = attach_working_context(dict(matches[0]), campaign)
    if task.get("dependency_artifacts", {}).get("C66-SUPPORT-INTERFACE") != "ATT-0120":
        raise RuntimeError("verified support interface ATT-0120 is not attached")
    if not (task.get("working_context") or {}).get("primary", {}).get("path"):
        raise RuntimeError("support-filtration primary working context is absent")
...
def main() -> int:
    with CampaignRunLock(CAMPAIGN_ID):
        ...
        ledger, ledger_path = _new_run_ledger(
            CAMPAIGN_ID, 1, ["cursor-grok"], [ENGINE], ["qwen", "claude"]
        )
        ...
        launch = {
            ...
            "note": "Normal v6 support-filtration proof attempt; not forced-proof.",
            "status": "running",
        }
