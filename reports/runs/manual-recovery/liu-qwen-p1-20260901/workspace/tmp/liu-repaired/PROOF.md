...
preprint statement and is not an axiom.
...
No statement about `M_{5,9}` is made. No Betti number is computed.
...
(The same lemma is not listed in Liu's Corollary 3.9, which cites only
Canning--Larson Lemmas 3.3, 3.4, 3.5, 3.7, 3.8.)
...
`M_\omega` is the rank-`\le 4` locus of `f_*\omega_f\to\bigoplus_{i=1}^8\sigma_i^*\omega_f`
(`e=5`, `f=8`, `k=4`). Expected Porteous codimension is
`(e-k)(f-k)=4`. Every fibre of `M_\omega\to M_5\setminus M^3_5` is an open in
`|\omega_C|=\mathbf P^4`, so `\dim M_\omega=12+4=16` inside the 20-dimensional
ambient `M_{5,8}`, and the actual codimension is 4. Porteous therefore applies.
Rational coefficients absorb determinantal multiplicity. The class is
tautological.
...
Liu states only that `h:BG\to B\mathrm{GL}_4` induces an isomorphism of
ordinary Chow rings (p. 12). That is the `X=\mathrm{Spec}\,\mathbf C` case of
Lemma 3.5 and is not by itself the CKgP. The affine-bundle permanence is the
missing universal step. (Liu knows the distinction: in Proposition 4.3, where
the stabilizer of `(H,p_9)` is a Levi with no unipotent radical, he drops
Lemma 3.5 from the citation list.)

### 2.5 Repair: Liu Proposition 3.3 is an asserted open immersion
...
(open in a Grassmann bundle) use this identification. We supply the inverse.

**Inverse.** A point of the proposed `W` is a net of quadrics in
`H^0(\mathcal O_{\mathbf P^4}(2))` over a point of `V'\subset(\mathbf P F)^7`,
cutting out a smooth complete intersection curve `C\subset\mathbf P^4`. The
`G`-structure recovers the hyperplane `H`. Then `C\cap H=(Q_1\cap Q_2\cap Q_3)|_H`
is a length-8 scheme; by Lemma 3.2 it is reduced, the first seven points are
the given points of `V'`, and the residual point is `p_8`. The complete
intersection of three quadrics in `\mathbf P^4` is canonically embedded of
genus 5, and `\mathcal O(\sum_{i=1}^8 p_i)=\mathcal O_C(1)=\omega_C`, so
`(C,p_1,\ldots,p_8)` lies in `M_\omega`.

**Openness.** Smooth complete intersection, reducedness of `C\cap H`, and
`p_8\notin\{p_1,\ldots,p_7\}` are open conditions, as in Liu's Proposition 2.5.
...
Lemma 3.8 sign repair (harmless): with `\mathbf P K=\mathrm{Proj}(\mathrm{Sym}^\bullet K^\vee)`,
twisting `K` by `L` twists `\mathcal O(1)` by `L^\vee`. Since
`G^\vee=L\otimes G'^\vee`, one has
...
This is the content of Liu's Corollary 3.9 after the four repairs: normalized
Chern formula for `G`; Lemma 3.6 for the gerbe; Lemma 3.5 for the unipotent
extension `BG`; and Lemma 9.9 plus stratification for gluing, together with
the inverse in Proposition 3.3.
...
`W_{24}H^{24}(M_{5,8};\mathbf Q)` is the image of `A^{12}(M_{5,8})`. Combined
with §4 this equals `RH^{24}(M_{5,8};\mathbf Q)`.

Ionel Theorem 0.1: for `g\ge 2` and `n>0`, any product of descendant or
tautological classes of degree `\ge g` vanishes on `M_{g,n}`. Here the Chow
codimension is `12\ge 5`, with slack 7. On the open moduli stack the
tautological ring is generated by `\psi` and `\kappa` (`\lambda` classes are
`\kappa`-polynomials, Liu Definition 1.1), so Ionel kills all of `R^{12}`.
Hence `W_{24}H^{24}(M_{5,8};\mathbf Q)=0`.

Both halves are needed: Lemma 4.3 alone yields only `cl(A^{12})`; Ionel alone
kills only the tautological summand.
...
of the three strata or in the gluing; a gap in CLP Lemma 4.3 or Ionel at
codimension 12; or a dimension/weight/twist error in the Borel--Moore
