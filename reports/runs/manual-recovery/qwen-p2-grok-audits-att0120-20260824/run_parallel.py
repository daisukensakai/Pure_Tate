#!/usr/bin/env python3
"""Parallel: Qwen P2 of ATT-0120 plus xAI Grok FAUD of FND-0180..0182.

FND-0168 is already corroborated and is not re-audited. One campaign lock.
Not C66-FULL. No model fallback.
"""
...
P2_TASK_ID = "TASK-V-ATT-0120-P2"
TARGET_ATTEMPT = "ATT-0120"
AUDIT_FINDING_IDS = ("FND-0180", "FND-0181", "FND-0182")
...
def main() -> int:
    ...
    if P2_ENGINE in set(p2.get("excluded_reviewer_engines") or []):
        raise RuntimeError("%s is excluded from %s" % (P2_ENGINE, P2_TASK_ID))
    ...
    try:
        with CampaignRunLock(CAMPAIGN_ID):
            ...
            ledger, ledger_path = _new_run_ledger(
                CAMPAIGN_ID, n_jobs, ["grok"], [P2_ENGINE], [P2_ENGINE, AUDIT_ENGINE]
            )
            jobs = []
            events = []
            ...
            launch = {
                "note": (
                    "parallel Qwen P2 of ATT-0120 plus xAI Grok FAUD of "
                    "FND-0180..0182; FND-0168 already corroborated; no model fallback"
                ),
                "jobs": jobs,
            ...
            notify(
                "Pure Tate • Qwen P2 + Grok audits starting",
                "%s • %s plus %s"
                % (
                    CAMPAIGN_ID,
                    p2_job["artifact_id"],
        ...
        notify("Pure Tate • Qwen P2 + Grok audits blocked", str(exc))
