...
META = Path("/tmp/pure-tate-two-reviews-meta.json")
...
def main():
    ...
    job = next(j for j in launch["jobs"] if j["artifact_id"] == rid)
    engine = job["engine"]
    task_id = job["task_id"]
    output = job["output"]
    res = Path(job["reservation"])
    log = vault / f"{rid}.{engine}.console.log"
    job["status"] = "running"
    _save(vault, job)
    notify(f"Pure Tate • {engine} review starting", f"C66-001 • {task_id} {job['target_attempt_id']} -> {rid}")
    with log.open("w") as fh:
        proc = subprocess.run(
            [sys.executable, "-m", "pure_tate", "agent-run",
             "--manifest", str(vault / f"run-{rid}.json"),
    ...
    if proc.returncode != 0 or not outp.exists():
        ...
        _save(vault, job)
        notify(f"Pure Tate • {engine} review failed", f"C66-001 • {rid} exit={proc.returncode}")
    ...
    try:
        ...
        job["ingest_warning"] = str(exc)
        print("ingest warning:", exc, flush=True)
    job["status"] = "done"
    job["result_verdict"] = payload.get("verdict")
    _save(vault, job)
    notify(f"Pure Tate • {engine} review done", f"C66-001 • {rid} {job['target_attempt_id']} P{job['review_pass']} verdict={payload.get('verdict')}")
    print("done", rid, payload.get("verdict"), flush=True)
