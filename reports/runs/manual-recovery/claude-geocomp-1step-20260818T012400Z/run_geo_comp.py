#!/usr/bin/env python3
"""One ordinary C66-GEO-COMP mathematics turn on Claude. No forced-proof.

Official campaign drive is not used: it would prioritize finding-audits and
then select qwen as the next GEO-COMP retry. This runner keeps the harness
contract (lock, recover, reserve, run_task, no overwrite, desktop+ntfy).
If Claude is rate-limited, do not hand the work to another model.
"""
...
def main() -> int:
    launch = {
        "note": "one ordinary GEO-COMP mathematics step; Claude/opus-5 effort=max; no forced-proof; no model fallback on rate limit",
        "stamp": "20260818T012400Z",
    ...
    try:
        with CampaignRunLock(CAMPAIGN_ID):
            ...
            _notify(
                "Pure Tate • Claude GEO-COMP starting",
