#!/usr/bin/env python3
"""Run Qwen's independent P1 review of ATT-0130."""
...
def current_task() -> dict:
    ...
    if len(matches) != 1 or matches[0].get("status") != "ready":
        raise RuntimeError("ATT-0130 has no ready P1 review task")
    task = dict(matches[0])
    if ENGINE in set(task.get("excluded_reviewer_engines") or []):
        raise RuntimeError("Qwen is not independent for ATT-0130")
...
def main() -> int:
    with CampaignRunLock(CAMPAIGN_ID):
        ...
        task = current_task()
        ledger, ledger_path = _new_run_ledger(CAMPAIGN_ID, 1, [ENGINE], ["cursor-grok"], [ENGINE])
