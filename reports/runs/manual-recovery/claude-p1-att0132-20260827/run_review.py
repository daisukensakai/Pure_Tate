#!/usr/bin/env python3
\"\"\"Run Claude's independent P1 review of Codex ATT-0132.\"\"\"
...
def current_task() -> dict:
    ...
    matches = [
        task for task in review_tasks(ATTEMPT_ID)
        if task.get(\"id\") == TASK_ID and task.get(\"review_pass\") == 1
    ]
    if len(matches) != 1 or matches[0].get(\"status\") != \"ready\":
        raise RuntimeError(\"ATT-0132 has no ready P1 review task\")
    task = dict(matches[0])
    if ENGINE in set(task.get(\"excluded_reviewer_engines\") or []):
        raise RuntimeError(\"Claude is not independent for ATT-0132\")
...
def main() -> int:
    with CampaignRunLock(CAMPAIGN_ID):
        ...
        ledger, ledger_path = _new_run_ledger(
            CAMPAIGN_ID, 1, [ENGINE], [\"cursor-grok\"], [ENGINE]
        )
        ...
        try:
            ...
            if event[\"state\"] == \"completed\" and output.is_file():
                ...
                spend_artifact_reservation(
                    reservation, reason=event[\"state\"], trace_id=event.get(\"trace_id\"), task_id=TASK_ID
                )
            ledger[\"events\"] = [event]
