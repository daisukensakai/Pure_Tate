#!/usr/bin/env python3
"""Resume interrupted Claude GEO-COMP session into a new ATT slot."""
...
META_PATH = Path("/tmp/pure-tate-claude-geocomp-resume-meta.json")
...
def main() -> int:
    ...
    if (ROOT / "proof/attempts/ATT-0075.json").exists():
        raise SystemExit("refusing: ATT-0075 unexpectedly exists")
    if not (ROOT / "proof/attempts/ATT-0076.json").exists():
        raise SystemExit("refusing: protected ATT-0076 missing unexpectedly")
    if output.exists():
        raise SystemExit("refusing to overwrite existing %s" % output)
    if artifact_id == "ATT-0075":
        raise SystemExit("refusing to write into spent ATT-0075")
    ...
    notify(
        ...
        "C66-001 • Claude --resume %s\nTASK-C66-M-003 -> %s (ATT-0075 closed)"
