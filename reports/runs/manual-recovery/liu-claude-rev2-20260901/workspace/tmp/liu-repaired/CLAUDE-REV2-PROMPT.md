# Independent mathematical review of Revision 2

Act as a hostile but fair expert referee in intersection theory and the
geometry of moduli spaces. This is an independent adversarial review of a
*twice-repaired proposed proof*. It is not a review of a published theorem
and not a vote of confidence in any prior AI report. Do not rely on any
prior AI review, campaign verdict, abstract, or authorial assertion. Do
not modify any file.

Read, in this order:

1. `tmp/liu-repaired/PROOF.md` — Revision 2 of the argument under review.
   Attack this.
...
Do not cite, quote, or use any Claude, Qwen, or Codex report.
...
- The *family* inverse for Liu Proposition 3.3 in `PROOF.md` §2.5: universal
  complete intersection `C`, finite-flat `C\cap\mathbf P F` of degree 8,
  residual section `p_8`, and the open `W`. Decide whether this is now a
  complete open immersion of stacks.
- Localization/gluing across `U_8`, `M_\omega`, and `M^3_{5,8}`, including
  Canning--Larson Lemma 9.9 at `(g,n)=(5,8)`.
- Porteous on `M_{5,8}\setminus M^3_{5,8}` rather than on all of `M_{5,8}`.
- Ionel as cohomological vanishing of the cycle-class image of `R^{12}`,
  not Chow vanishing.
- Expected codimension; Lemma 3.2; Grassmann presentations; Borel--Moore
  dimension, weight, and twist.
...
- `STRONGEST ATTACK`: the most serious possible flaw in Revision 2 and
  whether it survives scrutiny.
- `REPAIR ADJUDICATION`: for each repair, complete / incomplete / wrong,
  with locators. Pay special attention to the family inverse in §2.5.
