...
-- LEAN-CAMPAIGN LG7D16-003
-- LEAN-ATTEMPT LATT-0006
...
-- LEAN-WEIGHT Strong induction on 3g+n over a lowest-weight open BM carrier. The five critical endpoints are routed separately: the one-marking primitive-quotient argument at (3,12), (6,6), (7,4), the direct published CLP range at (4,10), and the repaired genus-five input at (5,8). CLP Table 1 bounds c(g) are carried independently of n-1 and select the route. Ionel-Looijenga is one general premise typed by genus, markings, codimension and cohomological level. Smooth-proper purity, kernel orientation, right exactness and proper same-degree duality remain separately typed.
...
-- LEAN-AXIOM ionel_looijenga_cohomological_vanishing => G7D16-OBL-02 -- the single general Ionel-Looijenga premise, typed by genus, markings, codimension and cohomological level

axiom genus_five_ckgp : vocab.GenusFiveCKgP
-- LEAN-AXIOM genus_five_ckgp => G7D16-OBL-01 -- Chow-Kunneth generation for M_5,8

axiom genus_five_tautological_chow : vocab.GenusFiveTautologicalChow
-- LEAN-AXIOM genus_five_tautological_chow => G7D16-OBL-01 -- repaired Chow equality A-star equals R-star, kept at Chow level

axiom genus_five_open_bm_conversion : vocab.GenusFiveOpenBMConversion
-- LEAN-AXIOM genus_five_open_bm_conversion => G7D16-OBL-01 -- CLP Lemma 4.3 pure-weight conversion followed by the exact Poincare twist to W_-16 H_16^BM

axiom genus_five_endpoint_from_repaired_inputs :
    vocab.GenusFiveCKgP ->
    vocab.GenusFiveTautologicalChow ->
    vocab.IonelLooijengaVanishes (endpointGroupIonelIndex endpointG5) ->
    vocab.GenusFiveOpenBMConversion ->
    vocab.GenusFiveEndpointVanishing
-- LEAN-AXIOM genus_five_endpoint_from_repaired_inputs => G7D16-OBL-01 -- assembly of the four distinct repaired premises, the third being general Ionel at genus five codimension twelve
...
-- LEAN-AXIOM clp_pure_tautological_at_endpoint => G7D16-OBL-02 -- CLP Table 1 and Proposition 4.5 at the endpoint pair itself, gated on n at most c(g)
...
-- LEAN-AXIOM clp_pure_tautological_at_smaller_pointed => G7D16-OBL-02 -- CLP Table 1 and Proposition 4.5 at the one-marking source, gated on n-1 at most c(g)
...
-- LEAN-AXIOM inclusive_ckgp_endpoint => G7D16-OBL-02 -- the pair is out of the published range while its one-marking source sits exactly at the CLP Table 1 endpoint
...
-- LEAN-AXIOM ionel_kills_critical_psi_source => G7D16-OBL-02 -- general Ionel at the psi-source codimension (k-2)/2 on the one-marking route
...
-- LEAN-AXIOM ionel_kills_endpoint_group => G7D16-OBL-02 -- general Ionel at the endpoint codimension k/2 on the direct published range
...
-- LEAN-AXIOM critical_endpoint_vanishes_one_marking => G7D16-OBL-02 -- one-marking primitive-quotient route, available only at (3,12), (6,6) and (7,4)
...
axiom critical_endpoint_vanishes_direct_range : forall e : CriticalEndpointIndex,
    ...
    vocab.IonelKillsEndpointGroup e ->
    vocab.CriticalEndpointVanishes e
-- LEAN-AXIOM critical_endpoint_vanishes_direct_range => G7D16-OBL-02 -- direct published Canning-Larson-Payne route, available only at (4,10)

axiom critical_endpoint_vanishes_repaired_genus_five : forall e : CriticalEndpointIndex,
    IsCriticalEndpoint e ->
    ...
    vocab.GenusFiveEndpointVanishing ->
    vocab.CriticalEndpointVanishes e
-- LEAN-AXIOM critical_endpoint_vanishes_repaired_genus_five => G7D16-OBL-02 -- repaired genus-five route, available only at (5,8) which lies outside the published range
...
-- LEAN-AXIOM open_bm_vanishes_strong_range => G7D16-OBL-03 -- explicit stronger zero range 2g+n at least eleven with its Ionel threshold codimension exposed
...
-- LEAN-AXIOM compact_base_genus_zero_one_two => G7D16-OBL-04 -- compact degree-sixteen base cases for every marking count
...
-- LEAN-AXIOM boundary_factor_complexity_decreases => G7D16-OBL-09 -- strict well-founded decrease of 3g+n at every factor
...
-- LEAN-AXIOM kunneth_types_product_factors => G7D16-OBL-06 -- Kunneth assembly consuming the odd and even low-degree compact inputs

axiom finite_graph_automorphism_invariants : vocab.FiniteGraphAutomorphismInvariants
-- LEAN-AXIOM finite_graph_automorphism_invariants => G7D16-OBL-06 -- rational finite graph-automorphism invariance, typed separately from Kunneth
...
-- LEAN-AXIOM boundary_image_tate_from_one_edge_graphs => G7D16-OBL-06 -- passage from the normalized boundary source to its distinct homological image carrier
...
-- LEAN-AXIOM smooth_proper_purity_input => G7D16-OBL-08 -- smooth-proper geometry input, typed separately from the purity it licenses
...
-- LEAN-AXIOM compact_homology_pure_polarizable => G7D16-OBL-08 -- purity follows from the smooth-proper input plus the purity tag
...
theorem genusFiveEndpointInput : vocab.GenusFiveEndpointVanishing :=
  genus_five_endpoint_from_repaired_inputs genus_five_ckgp genus_five_tautological_chow
    (endpointGroupIonel endpointG5 (by decide) (by decide) (by decide))
    genus_five_open_bm_conversion

theorem endpointG5Vanishes : vocab.CriticalEndpointVanishes endpointG5 :=
  critical_endpoint_vanishes_repaired_genus_five endpointG5 (by right; right; left; rfl)
    rfl rfl (by decide) (by decide) genusFiveEndpointInput
