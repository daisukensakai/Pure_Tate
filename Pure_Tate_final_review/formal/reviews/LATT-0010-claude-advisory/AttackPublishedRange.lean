...
-- ATTACK 4: delete the published-range route (G7D16-OBL-02 / AX17).
...
def openTateModel (i : OpenBMIndex) : Prop := openVanishModel i
...
def stable30b : StablePairIndex := { genus := 3, markings := 0 }
theorem stable30bStable : IsStablePair stable30b := (by decide : (3 : Nat) <= 2 * 3 + 0)

theorem attack4_models_all_but_ax17 :
    AX01 ∧ AX02 ∧ AX03 ∧ AX04 ∧ AX05 ∧ AX06 ∧ AX07 ∧ AX08 ∧ AX09 ∧ AX10 ∧
    AX11 ∧ AX12 ∧ AX13 ∧ AX14 ∧ AX15 ∧ AX16 ∧ AX18 ∧ AX19 ∧ AX20 ∧
...
theorem attack4_refutes_ax17 : ¬ AX17 := by
  intro h
  have hv := h stable30b ⟨stable30bStable, by decide, by decide, by decide, by decide⟩
    ⟨stable30bStable, by decide, by decide, by decide⟩
  rcases hv.2.2 with hw | hc | hb
  · exact absurd hw.2.2.2 (by decide)
  · exact absurd hc.1 (by decide)
  · exact absurd hb.2.2.1 (by decide)

theorem attack4_target_fails_at_30 :
    ¬ CompactH16IsFiniteTateSum (compactH16Target stable30b) := by
  refine compactTargetFailsWithoutOpen openTateModel stable30b (by decide) ?_
  rintro ⟨_, _, hw | hc | hb⟩
  · exact absurd hw.2.2.2 (by decide)
  · exact absurd hc.1 (by decide)
  · exact absurd hb.2.2.1 (by decide)

#print axioms attack4_models_all_but_ax17
#print axioms attack4_refutes_ax17
#print axioms attack4_target_fails_at_30
