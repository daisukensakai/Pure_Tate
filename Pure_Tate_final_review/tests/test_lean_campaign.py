...
class LeanCampaignTests(unittest.TestCase):
    ...
    def test_valid_attempt_elaborates_and_report_is_reproducible(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self._patches(root, formal, campaigns, attempts, reviews):
                ...
                self.assertEqual(result.errors, [])
                self.assertEqual(report["result"], "PASS")
                self.assertEqual(report["prover_engines"], ["prover"])
    ...
    def test_multiple_prover_engines_are_all_nonindependent(self):
        self.assertEqual(
            set(
                lean_campaign._prover_engines(
                    {
                        "prover_engine": "codex",
    ...
    def test_manifest_prover_engines_must_include_primary(self):
        with tempfile.TemporaryDirectory() as temporary:
            fixture = self._fixture(temporary)
            root, formal, campaigns, attempts, reviews, directory, _campaign = fixture
            manifest_path = directory / "manifest.json"
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
            manifest["prover_engines"] = ["claude"]
            self._write_json(manifest_path, manifest)
            with self._patches(root, formal, campaigns, attempts, reviews):
                result, _report = lean_campaign.check_attempt("LATT-0001")
            self.assertIn(
                "manifest prover_engines must include prover_engine", result.errors
            )

    def test_confirmed_review_by_secondary_prover_is_rejected(self):
        with tempfile.TemporaryDirectory() as temporary:
            fixture = self._fixture(temporary)
            root, formal, campaigns, attempts, reviews, directory, campaign = fixture
            manifest_path = directory / "manifest.json"
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
            manifest["prover_engines"] = ["prover", "review-a"]
            self._write_json(manifest_path, manifest)
            campaign["review_policy"] = {
                "reviewers_must_differ_from_prover": True,
            }
            self._write_json(campaigns / "LC66-001.json", campaign)
            with self._patches(root, formal, campaigns, attempts, reviews):
                result, report = lean_campaign.check_attempt("LATT-0001", write=True)
                ...
                review_result = lean_campaign.validate_review(
                    review, campaign, report_path
                )
            self.assertIn(
                "LREV-0001 reviewer is a contributing prover",
